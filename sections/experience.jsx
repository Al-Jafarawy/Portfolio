...
const FreelanceExperience = () => {
  ...
  const itemVariants = {
    hidden: { opacity: 0, y: 100, scale: 0.8 }, // بعيد و صغير
    visible: {
      opacity: 1,
      y: 0,
      scale: 1,
  ...
  return (
    <Fragment>
      <section className="max-w-7xl mx-auto my-10 px-5 md:px-10 py-10">
        ...
        <motion.div
  className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-6 justify-center items-start"
  initial="hidden"
  ...
  <motion.a
    className={`flex flex-col justify-center items-center gap-2 p-4 rounded-xl shadow-md hover:shadow-lg transition-all duration-300 bg-white dark:bg-gray-800 ${
      idx === platforms.length - 1 && platforms.length % 2 !== 0
        ? "col-start-1 col-end-3 justify-self-center px-14 py-5"
        : ""
    }`}
    href={platform.link}
    key={idx}
